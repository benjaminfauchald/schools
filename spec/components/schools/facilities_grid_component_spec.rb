...
RSpec.describe Schools::FacilitiesGridComponent, type: :component do
  let(:academic_facility) { double('Term', slug: 'library', name: 'Library') }
  let(:sports_facility) { double('Term', slug: 'gymnasium', name: 'Gymnasium') }
  let(:tech_facility) { double('Term', slug: 'computer_lab', name: 'Computer Lab') }
  let(:uncategorized_facility) { double('Term', slug: 'custom_facility', name: 'Custom Facility') }
  ...
  describe 'render behavior' do
    ...
    it 'renders when facilities exist but none match any category' do
      empty_facility = double('Term', slug: 'unknown_facility', name: 'Unknown')
  ...
  describe '#total_facilities_count' do
    it 'returns correct count for multiple facilities' do
      ...
      expect(component.total_facilities_count).to eq(3)
    end

    it 'returns zero for empty facilities' do
      component = described_class.new(facilities: [])
      expect(component.total_facilities_count).to eq(0)
    end
  end
  ...
  describe '#facility_groups' do
    ...
    it 'includes all 8 possible categories' do
      ...
      all_facilities = [
        double('Term', slug: 'library', name: 'Library'),                    # Academic
        double('Term', slug: 'gymnasium', name: 'Gymnasium'),                # Sports
        double('Term', slug: 'cafeteria', name: 'Cafeteria'),               # Dining
        double('Term', slug: 'medical_clinic', name: 'Medical Clinic'),      # Health
        double('Term', slug: 'computer_lab', name: 'Computer Lab'),          # Tech
        double('Term', slug: 'art_studio', name: 'Art Studio'),             # Arts
        double('Term', slug: 'parking_lot', name: 'Parking Lot'),           # Services
        double('Term', slug: 'custom_room', name: 'Custom Room')            # Other
      ]
  ...
  describe 'rendering' do
    it 'renders the component with facilities' do
      ...
      render_inline(described_class.new(facilities: facilities))

      expect(rendered_component).to have_css('.bg-white')
      expect(rendered_component).to have_content('Campus Facilities')
      expect(rendered_component).to have_content('Academic Facilities')
    end
    ...
    it 'renders facility counts correctly' do
      ...
      expect(rendered_component).to have_content('Total Facilities')
      expect(rendered_component).to have_content('3') # total count
    end

    it 'renders TagBadgeComponent for each facility' do
      render_inline(described_class.new(facilities: [ academic_facility ]))

      # Should render the TagBadgeComponent (we can't easily test the component itself in unit tests)
      expect(rendered_component).to have_css('.flex-wrap') # container for badges
    end
